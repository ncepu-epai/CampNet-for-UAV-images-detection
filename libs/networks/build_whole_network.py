...
class DetectionNetwork(object):
    ...
    def roi_pooling(self, feature_maps, rois, img_shape, scope):
        ...
        with tf.variable_scope('ROI_Warping_' + scope):
            ...
            normalized_fx1 = fx1 / img_w
            normalized_fx2 = fx2 / img_w
            normalized_fy1 = fy1 / img_h
            normalized_fy2 = fy2 / img_h

            normalized_front_rois = tf.transpose(
                tf.stack([normalized_fy1, normalized_fx1, normalized_fy2, normalized_fx2]),
                name='get_normalized_front_rois')
            normalized_front_rois = tf.stop_gradient(normalized_front_rois)

            cropped_front_roi_features = tf.image.crop_and_resize(feature_maps, normalized_front_rois,
                                                                  box_ind=tf.zeros(shape=[N, ],
                                                                                   dtype=tf.int32),
                                                                  crop_size=[cfgs.ROI_SIZE, cfgs.ROI_SIZE],
                                                                  name='context_CROP_AND_RESIZE'
                                                                  )
            # context_r=0.5
            front_roi_features = slim.max_pool2d(cropped_front_roi_features,
                                                 [cfgs.ROI_POOL_KERNEL_SIZE, cfgs.ROI_POOL_KERNEL_SIZE],
                                                 stride=cfgs.ROI_POOL_KERNEL_SIZE)
            context_r = 0.5
            context_roi_features = (1 - context_r) * roi_features + context_r * front_roi_features

        # return roi_features
        return context_roi_features
